...
<div class="modal fade" tabindex="-1" id="raportkarakter-modal">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        ...
        <div class="modal-body" id="raportkarakter-modal-content">
          
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
          <button type="button" class="btn btn-primary" id="download-excel-penilaian">Download Excel</button>
        </div>
      </div>
    </div>
  </div>
    ...
    $(document).ready(function() {
        ...
        $(document).delegate(".add-penilai", 'click', function(e) {
            ...
            <div class="list-guru-main d-inline-block m-1" data-id='${$(this).data('id')}'>   
                                <input class='input-list-guru' type='hidden' name='listguru[]' value='${$(this).data('id')}'>
        ...
        $("#raportkarakter-form").submit(function(e){
            
            e.preventDefault();
            var listguru = $(".input-list-guru").map(function(e){
                return $(this).val();
            }).get();
            ...
            $.ajax({
                ...
                data: {
                    ...
                    semuaguru: $("input[name=semuaguru]").val(),
                    listguru: listguru
                },
                type: "get",
                success: function(response){
                    Swal.hideLoading()
                    alert(response);
                    $("#raportkarakter-modal-content").html(response);
                    modal.show()
        ...
        $("#download-excel-penilaian").click(function(){
            var listguru = $(".input-list-guru").map(function(e){
                return $(this).val();
            }).get();

            $.ajax({
                url: "{{route('eraport.downloadraportkarakter')}}",
                data: {
                    angkatan: $("select[name=angkatan]").val(),
                    jurusan: $("select[name=jurusan]").val(),
                    dari: $("input[name=dari]").val(),
                ...
                type: "GET",
                success: function(){
                    Swal.fire("Excel Berhasil didownload")
                }
            })   
        })
