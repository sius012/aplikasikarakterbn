    ...
    <div class="modal fade" tabindex="-1" id="modal-pengajuan-ha">
        <div class="modal-dialog">
          <div class="modal-content">
            <form action="{{route('guru.ajukanhakakses.store')}}">
            @csrf
              ...
              <div class="mb-3">
                ...
                <select name="" id="" class="form-select">
                    @foreach($angkatan as $a => $akt)
                        <option value="{{$akt->id_angkatan}}" name="angkatan">{{$akt->id_angkatan}}</option>
                    @endforeach
                </select>
              </div>
              <div class="mb-3">
                ...
                <select name="" id="" class="form-select">
                    @foreach($jurusan as $j => $jrs)
                        <option value="{{$jrs->id_jurusan}}" name="jurusan">{{$jrs->jurusan}}</option>
                    @endforeach
                </select>
              </div>
              <div class="row">
