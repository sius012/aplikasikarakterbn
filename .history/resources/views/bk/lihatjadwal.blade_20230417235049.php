    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive" style="overflow-x:scroll;">
                <table class="container" style="width: 3000px">
                    <thead class="row">
                        
                        @foreach($jadwalkonseling["detail"] as $i=> $jk)
                        <div class="col p-4 fw-bolder">{{getDayName($i)}}</div>
                        @endforeach
                    </thead>
                    @for($j = 0;$j < 5;$j++) <div class="row">
                        @foreach($jadwalkonseling["detail"] as $k => $jk)
                        @if(isset($jk[$j]))
                        <div class="col" @if($jk[$j]->bookedby_count > 0) data-status="booked" @endif>
                            <div class="session-card @if($jk[$j]->bookedby_count > 0) @if($jk[$j]->bookedby->status=="Selesai") done @elseif($jk[$j]->status=="Dipesan") booked @else reschedule @endif @endif">
                                @if($jk[$j]->bookedby_count > 0)
                                <div class="position-relative">
                                    <div class="card card-info mb-3 position-absolute" style="width: 400px;">
                                        <div class="row g-0">
                                            <div class="col-md-3">
                                                <img src="{{$jk[$j]->bookedby->pemesan->getimageurl()}}" class="img-fluid rounded-start" alt="...">
                                            </div>
                                            <div class="col-md-8">
                                                <div class="card-body p-2">
                                                    <h5 class="card-title">{{$jk[$j]->bookedby->pemesan->nama_siswa}}</h5>
                                                    <p class="card-text">{{$jk[$j]->bookedby->keterangan_siswa}}</p>
                                                    <div class="container">
                                                        @if(in_array($jk[$j]->bookedby->status,["Dipesan","Reschedule"]))
                                                        <button type="button" class="btn btn-warning m-0 btn-edit-jadwal p-3 py-1" value="{{$jk[$j]->id_djk}}"><i class="fa fa-edit"></i></button>
                                                        <button type="button" class="btn btn-success m-0 btn-reserv-jadwal p-3 py-1" value="{{$jk[$j]->id_djk}}" data-idb="{{$jk[$j]->bookedby->id_bk}}"><i class="fa fa-check"></i></button>
                                                        <button type="button" class="btn btn-danger m-0 btn-batal-jadwal p-3 py-1" value="{{$jk[$j]->id_djk}}"><i class="fa fa-trash"></i></button>
                                                        @else
                                                        <button type="button" class="btn btn-info m-0 btn-info-sesi p-3 py-1" value="{{$jk[$j]->id_djk}}" data-ibk="{{$jk[$j]->bookedby->id_bk}}"><i class="fa fa-info"></i></button>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input type="date" class="form-control" id="info-date" readonly>
          </div>
          <div class="form-group">
            <label for="" class="form-label">
                Konseli
            </label>
            <input type="text" class="form-control" id="info-konseli" readonly>
          </div>
          <div class="form-group">
            <label for="" class="form-label">
                Pertemuan Ke
            </label>
            <input type="text" class="form-control" id="info-pertemuanke" readonly>
          </div>
          <div class="row">
            <div class="col"><div class="form-group">
                ...
                <input type="time" class="form-control" id="info-dari" readonly>
              </div></div>
            <div class="col">
                <label for="" class="form-label">
                    Sampai
                </label>
                <input type="time" class="form-control" id="info-sampai" readonly>
            </div>
          </div>
          <div class="form-group">
            <label for="" class="form-label">
            ...
            <input type="text" class="form-control" id="info-ks" readonly>
          </div>
          <div class="form-group">
            <label for="" class="form-label">
                Status
            </label>
            <input type="text" class="form-control" id="info-status" readonly>
          </div>
          <div class="form-group">
            <label for="" class="form-label">
               Keterangan Reschedule
            </label>
            <input type="text" class="form-control" id="info-reschedule" readonly>
          </div>
          <div class="form-group mt-5">
            <label for="" class="form-label">
                Tempat
            </label>
            <input type="text" class="form-control" id="info-tempat">
          </div>
          <div class="form-group">
            <label for="" class="form-label">
                Pendekatan yang digunakan
            </label>
            <input type="text" class="form-control" id="info-pdg">
          </div>
          <div class="form-group">
            <label for="" class="form-label">
                Catatan Konselor
            </label>
            <input type="text" class="form-control" id="info-ck">
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        ...
        $(".btn-info-sesi").click(function(){
            $.ajax({
                ...
                success: function(data){
                    var inforeschedule = $("#info-reschedule").closest(".form-group");
                    console.log(data)
                    $("#info-date").val(getDateFromString(data.detail_j_k.tanggal))
                    $("#info-konseli").val(data.pemesan.nama_siswa);
                    $("#info-tempat").val(data.tempat);
                    $("#info-pdg").val(data.pdg);
                    $("#info-pertemuanke").val(data.detail_j_k.pertemuan_ke);
                    $("#info-reschedule").val(data.detail_j_k.keterangan_reschedule);
                    $("#info-status").val(data.status);
                    $("#info-ck").val(data.catatan_konselor);
                    $("#info-ks").val(data.keterangan_siswa);
                    if(data.keterangan_reschedule == null){
                        inforeschedule.hide()
                    }
                
                    $("#info-dari").val(data.detail_j_k.dari);
                    $("#info-dari").val(data.detail_j_k.dari);
                    $("#info-dari").val(data.detail_j_k.dari);
                    $("#info-sampai").val(data.detail_j_k.sampai);
                    modalinfosesi.show()
                },error: function(err){
