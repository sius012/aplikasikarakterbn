...
@section('content')
<form action="{{route('bk.')}}" method="POST">
    @csrf
<div class="card">
    ...
    <div class="card-body">
        <div class="form-group">
            ...
            <input type="text" class="form-control" value="{{$jadwalkonseling["jadwal_konseling"]->keterangan}}">
        </div>
        <div class="form-group">
            <label for="" class="form-label">
                Bulan
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" value="{{$jadwalkonseling["jadwal_konseling"]->minggu}}">
        </div>
                    ...
                    @for($j = 0;$j < 5;$j++) <tr class="field-input-jam">
                        ...
                        <td class="p-3 @if($jk[$j]->bookedby_count > 0) border-booked @endif" data-status="{{$jk[$j]->bookedby_count > 0 ? "booked" : "ready"}}" data-html="true" data-html="true">
                                ...
                                <div class="card-body p-2">
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            ...
                                            <button class="btn btn-success btn-reserv-jadwal p-3 py-1" value="{{$jk[$j]->id_djk}}" data-idb="{{$jk[$j]->bookedby->first()->id_bk}}"><i class="fa fa-check"></i></button>
                            ...
                            <div class="row">
                                <div class="col">
                                    Dari
                                    <input type="time" @if($jk[$j]->bookedby_count < 1) name="djk[{{$jk[$j]->id_djk}}]" @endif class="form-control  jam-field" placeholder="" value="{{$jk[$j]->dari}}" readonly></div>
                                <div class="col">
                                    Sampai
                                    <input type="time" @if($jk[$j]->bookedby_count < 1)name="djk[{{$jk[$j]->id_djk}}]" @endif class="form-control  jam-field" value="{{$jk[$j]->sampai}}" readonly></div>
                            </div>
                        </td>
                        @else
                        <td></td>
