...
<div class="row">
    <div class="col-sm-4">
        ...
        <form action="{{route('datasiswa.updatepp')}}" method="POST">
        @csrf
        <label for="fileInput">@if($siswa->foto_profil != null) Perbarui Foto Profil @else Lengkapi Foto Profil @endif</label>
        <input type="file" id="fileInput" name="fileInput" accept=".jpg, .png" class="form-control">
        <button type="submit" class="btn btn-primary">@if($siswa->foto_profil != null) Perbarui Foto Profil @else Lengkapi Foto Profil @endif</button>
    </form>
    </div>
    <div class="col-sm-8">
        <div class="container">
