...
$(document).ready(function () {
    $(document).delegate(".download-pdf", "click", function () {
        ...
        XLSX.utils.sheet_add_json(excelFile.Sheets["sheet1"], [["Laporan "]], {
            origin: -4,
            skipHeader: true,
        });

        const dataBarisAsal = excelFile.Sheets['sheet1'].splice(1 - 1, 1)[0];
        excelFile.Sheets['sheet1'].splice(4 - 1, 0, dataBarisAsal);

        for (let i = worksheet['!ref'].e.r; i >= worksheet['!ref'].s.r; i--) {
          ...
          XLSX.writeFile(workbook, 'file_setelah_digeser.xlsx');
          Dengan menggunakan loop untuk menggeser tabel ke bawah, kita dapat memindahkan data di dalam tabel ke baris yang lebih rendah sesuai dengan jumlah baris yang ditentukan dalam variabel jumlahBarisGeser.
