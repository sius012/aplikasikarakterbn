    ...
    public function entryTheClass($angkatan, $jurusan){
        if(!regularPermission($this->getRoleNames(),["Admin","Kepala Sekolah"])){
            ...
            foreach($hakAkses as $h => $ha){
                ...
                //Check Agama 
                if(in_array($angkatan,explode("id_angkatan")) and in_array($jurusan, $id_jurusan))
            ...
            if($checkAngkatan and $checkJurusan and $checkAngkatanWK and $checkJurusanWK){
                ...
                $siswa = Siswa::where("id_angkatan", $angkatan)->where("id_jurusan", $jurusan)->whereIn("jenis_kelamin", $realJK)->get();
                return $siswa;
            }else{
                return false;
            }
