...
class TeacherHasTeaching extends Model
{
    ...
    protected $fillable = [
        "id_guru","id_jurusan",'id_angkatan',"sebagai","dari","sampai","status","agama","jenis_kelamin"
    ];
    ...
    public function angkatan(){
        if($this->id_angkatan != "semua"){
            ...
            foreach($angkatan as $i => $akt){
                array_push($angkatanstr,"Kelas ".$akt->kelas());
            }
            return arrayComa($angkatanstr);
        }else{
            return "Semua Kelas";
        }
