...
class User extends Authenticatable
{
    ...
    public function entryTheClass($angkatan, $jurusan, $params = null){
        if(!regularPermission($this->getRoleNames(),["Admin","Kepala Sekolah"])){
            ...
            foreach($jk as $j => $jj){
                ...
                $jk[$j]["jurusan"] = call_user_func_array("array_merge", $jk[$j]["jurusan"]);
            }
            ...
            if($checkAngkatan and $checkJurusan and $checkAngkatanWK and $checkJurusanWK){
                ...
                $siswa = Siswa::where("id_angkatan", $angkatan)->where(function($q) use($angkatan){
                    $q->where("id_angkatan",$angkatan)->where("id_ca","!=", null)->orWhere(function($b) use($angkatan){
                        $b->where("id_ca", $angkatan)
                    });
                })->where("id_ca", $angkatan)->whereIn("jenis_kelamin", $realJK)->whereIn("agama", $agama);
    ...
    public function checkStudentProfile($nis){
        ...
        $checkClass = $this->entryTheClass($angkatan, $jurusan, ["nis"=>$nis,"singular"=>"yes"]);
