    ...
    public function entryTheClass($angkatan, $jurusan){
        if(!regularPermission($this->getRoleNames(),["Admin","Kepala Sekolah"])){
            ...
            foreach($hakAkses as $h => $ha){
                $angkatanList = $ha->id_angkatan == "semua" ? Angkatan::get()->pluck('id_angkatan') : explode(",",$ha->id_angkatan);
                array_push($angkatanListMain,$angkatanList);

                $jurusanList = $ha->id_jurusan == "semua" ? Jurusan::get()->pluck('id_jurusan') : explode(",",$ha->id_jurusan);
                array_push($jurusanListMain,$angkatanList);
            }
