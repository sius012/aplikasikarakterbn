...
class HomeController extends Controller
{
    public function beranda(Request $req)
    {
        ...
        if($req->has("dari") and $req->has("sampai")){
            dd(Carbon::parse($req->dari)->format("Y-m-d H:i:s"));
            $cs = $cs->whereBetween("tanggal", [Carbon::parse($req->dari)->format("Y-m-d H:i:s"),Carbon::parse($req->sampai)->format("Y-m-d H:i:s")]);
        }
