...
class SuperAppController extends Controller
{
    ...
    public function profilsiswa($nis)
    {
        ...
        $catatanPositif = CatatanSikap::where("tindakan", "Positif")->where("nis_siswa",$nis)->get();
        $catatanNegatif = CatatanSikap::where("tindakan", "Negatif")->where("nis_siswa",$nis)->get();

        $catatankonseling = PengajuanKonseling::where("nis_siswa", $nis)->where("status","Selesai")->get();

        $siswa = Siswa::with(["detail","alamat"])->find($nis);

        return view("datasiswa.profilsiswa", ["siswa" => $siswa, "catataneraport"=>$catatanEraport, "catatankonseling"=>$catatankonseling, "catatanNegatif"=>$catatanNegatif, "catatanNegatif"=>$catatanPositif]);
    }
