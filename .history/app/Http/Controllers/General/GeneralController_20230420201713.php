...
class GeneralController extends Controller
{
    ...
    public function injectphotoprofile()
    {
        ...
        $files = File::directories(public_path("siswa/"));
        foreach ($files as $fm) {
            ...
            foreach (File::directories($fm) as $file) {
                ...
                $jurusana = $arrjurusan[$keyj];
                $jurusan = $jurusana[$key]
                foreach (File::files($file) as $i => $j) {
                    ...
                    if ($siswa) {
                        ...
                        // $imageCompressed = Image::make($j);
                        //  $imageCompressed->resize(360,480)->save(public_path("siswa/" . $angkatan . "/" . $jurusandata->jurusan . "/" . $name));
                    }
