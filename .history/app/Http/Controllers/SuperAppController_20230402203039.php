...
class SuperAppController extends Controller
{
    ...
    public function pengajuankonseling(Request $req){
        ...
        foreach($konselor as $ko){
            $jadwalmingguini = DetailJK::whereHas("jadwal", function($q){
                $q->where("tahun", Carbon::now()->year)->where("bulan",1)->where("minggu", Carbon::now()->weekOfMonth);
            })->where("hari", Carbon::now()->dayOfWeek)->get();
