        ...
        foreach ($files as $fm) {
            ...
            foreach (File::directories($fm) as $file) {
                ...
                foreach (File::files($file) as $i => $j) {
                    ...
                    $siswa = Siswa::where("id_jurusan",$jurusandata->id_jurusan)->where("id_angkatan", $angkatan)->where("no_absen", $i+1)->first();
                    if($siswa){
                        $name = .$siswa->id_jurusan."_".$siswa->id_angkatan$siswa->nis."_"."."."jpg";
                        $siswa->foto
                        Storage::disk("public_disk")->put("siswa/".$angkatan."/".$jurusandata->jurusan."/".$siswa->nis."_".$siswa->id_angkatan."."."jpg", file_get_contents($j));
                    }
