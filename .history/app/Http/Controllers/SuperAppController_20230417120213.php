...
class SuperAppController extends Controller
{
    ...
    public function datajurusan($angkatan)
    {
        ...
        dd(terdapatpada(Auth::user()->getRoleNames()->toArray(),["Admin","Guru BK","Wali Kelas"]));
    //    dd();
        ...
        $jurusan = $jurusan->map(function($e,$i) use($angkatan) {
            ...
            //Cek Apakah User memiliki hak Akses Wali Kelas untuk Jurusan ini
            $hakakses = TeacherHasTeaching::where("id_jurusan",$i)->where("id_angkatan",$angkatan)->where("sebagai", "LIKE","%Wali Kelas%")->count();
            $visibility = false;
            if(terdapatpada(Auth::user()->getRoleNames()->toArray(), ["Admin","Guru BK"])){
            ...
            return ["data"=>$e,"visibility"=>$visibility, "hakakes"=>$hakakses];
