...
class SuperAppController extends Controller
{
    ...
    public function datasiswa(Request $req)
    {
        ...
        foreach ($angkatan as $i => $akt) {
            ...
            if ($p > 0 or in_array($therole, $exceptedRole)) {
                $kelas[$akt->kelas()] =
                    [
        ...
        return view("datasiswa.index", ["kelas" => $kelas, "siswa" => $siswa]);
    }

    public function siswakelas($angkatan, $jurusan)
    {
        $siswa = Siswa::where("id_angkatan", $angkatan)->where("id_jurusan", $jurusan)->orderBy("no_absen", "desc")->get();
    ...
    public function profilsiswa($nis)
    {
        ...
        $kategori_negatif = $this->kategori(["tindakan" => "negatif"]);
        $kategori_positif = $this->kategori(["tindakan" => "positif"]);

        $catatanPositif = CatatanSikap::with("siswa")->where("nis_siswa", $nis)->with(["penilai", "lampiran"])->whereHas("kategori", function ($q) {
            $q->where("tindakan", "Positif");
        })->get();
        $catatanNegatif = CatatanSikap::with("siswa")->where("nis_siswa", $nis)->with(["penilai", "lampiran"])->whereHas("kategori", function ($q) {
        ...
        return view("datasiswa.profilsiswa", ["siswa" => $siswa, "kategori_negatif" => $kategori_negatif, "kategori_positif" => $kategori_positif, "catatan_positif" => $catatanPositif, "catatan_negatif" => $catatanNegatif]);
    }
    ...
    public function rekaperaport($angkatan, $jurusan)
    {
        ...
        for ($i = 1; $i <=  $jumlahbulan; $i++) {
            ...
            //BIG QUERY FOR ALL
            $queryPA = PenilaianGuru::where("id_angkatan", $angkatan)->where("id_jurusan", $jurusan)->whereMonth("tanggal_penilaian", $i)->whereYear("tanggal_penilaian", date("Y"))->whereHas("penilai.roles.role", function ($q) {
                $q->where("name", "Pamong Putra");
            });
            $queryPI = PenilaianGuru::where("id_angkatan", $angkatan)->where("id_jurusan", $jurusan)->whereMonth("tanggal_penilaian", $i)->whereYear("tanggal_penilaian", date("Y"))->whereHas("penilai.roles.role", function ($q) {
                $q->where("name", "Pamong Putri");
            });

            $querySekolah = PenilaianGuru::where("id_angkatan", $angkatan)->where("id_jurusan", $jurusan)->whereMonth("tanggal_penilaian", $i)->whereYear("tanggal_penilaian", date("Y"))->whereHas("penilai.roles.role", function ($q) {
                $q->whereNotIn("name", ["Pamong Putri", "Pamong Putra"]);
            });

            $arr["pg"]["asrama_putra"]["data"] = $queryPA->get();
            $arr["pg"]["asrama_putra"]["jumlah"] = $queryPA->count();

            $arr["pg"]["asrama_putri"]["data"] = $queryPI->get();
            $arr["pg"]["asrama_putri"]["jumlah"] = $queryPI->count();
    ...
    {
        $array = Excel::toArray(new Raport, $req->file("file"));
