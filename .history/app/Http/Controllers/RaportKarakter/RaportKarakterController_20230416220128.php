...
class RaportKarakterController extends Controller
{
    ...
    public function rekaperaportdetail(array $params)
    {
        ...
        $walikelas = TeacherHasTeaching::where("id_jurusan",$jurusan)-> where("id_angkatan",$angkatan)->whereHas("guru.roles.role", function($q) use($jurusanril,$angkatan){
            $q->where("name","Wali Kelas");
        })->where("sebagai","LIKE",$angkatan." ".$jurusanril->jurusan)->where("dari",">",$dari);
        ...
        return ["siswa" => $siswa, "rekaplist" => $rekaplist, "kelas" => $kelas, "jurusan" => $jurusan->jurusan,"semester"=>$semester, "w"];
    }
}
