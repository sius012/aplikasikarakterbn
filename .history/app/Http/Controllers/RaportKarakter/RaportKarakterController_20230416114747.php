...
class RaportKarakterController extends Controller
{
    ...
    public function downloadraportkarakter(Request $req)
    {
        if (Session::has('listpenilaian')) {
            ...
            $name = "cache-".date('Y-m-d H:i:s').".x
            $filePath = 'exports/example.xlsx'; // Specify the desired file path within the storage directory
            Excel::store(new Master(Session::get('listpenilaian')), $filePath, 'public');
