...
class RaportKarakterController extends Controller
{
    ...
    public function downloadraportkarakter(Request $req)
    {
        if (Session::has('listpenilaian')) {
            ...
            $spreadsheet->getActiveSheet()->getPageSetup()->setFitToWidth(Session::get('listpenilaian')['siswa']->count()+1);
            $spreadsheet->getActiveSheet()->getPageSetup()->setFitToPage(true);

            foreach(Session::get('listpenilaian')['siswa'] as $i => $s){
                $spreadsheet->getActiveSheet()->getColumnDimension(convertToExcelColumn(($i*7)+1))->setWidth(20);
                $spreadsheet->getActiveSheet()->getColumnDimension(convertToExcelColumn(($i*7)+3))->setWidth(40);
                $spreadsheet->getActiveSheet()->getColumnDimension(convertToExcelColumn(($i*7)+4))->setWidth(10);
                $spreadsheet->getActiveSheet()->getColumnDimension(convertToExcelColumn(($i*7)+5))->setWidth(10);
                $spreadsheet->getActiveSheet()->getColumnDimension(convertToExcelColumn(($i*7)+6))->setWidth(10);
                $spreadsheet->getActiveSheet()->getColumnDimension(convertToExcelColumn(($i*7)+7))->setWidth(10);
            }
