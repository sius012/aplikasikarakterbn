...
class RaportKarakterController extends Controller
{
    ...
    public function rekaperaportdetail(array $params)
    {
       ...
       // $jurusanril = Jurusan::find($jurusan);
        ...
        $walikelas = TeacherHasTeaching::where("id_jurusan",$jurusan)-> where("id_angkatan",$angkatan)->whereHas("guru.roles.role", function($q) use($jurusan,$angkatan){
            $q->where("name","Wali Kelas");
        })->where("sebagai","LIKE",$angkatan." ".$jurusan->jurusan)->where("dari",">",$dari)->first();
