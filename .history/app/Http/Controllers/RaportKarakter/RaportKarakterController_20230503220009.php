...
class RaportKarakterController extends Controller
{
    ...
    public function rekaperaportdetail(array $params)
    {
       ...
       // $jurusanril = Jurusan::find($jurusan);
        ...
        //Rincian
        $rincian = $this->rincian($params);


        return ["siswa" => $siswa, "rincian"=>$rincian,"rekaplist" => $rekaplist, "kelas" => $kelas, "jurusan" => $jurusan->jurusan,"semester"=>$semester, "walikelas"=>$walikelas];
    }


    public function rincian($params){
        $angkatan = $params["angkatan"];
        $jurusan = $params["jurusan"];
        $dari = $params["dari"];
        $sampai = $params["sampai"];
        $siswaN1 = DetailPG::with("siswa")->whereHas("siswa", function($q) use($jurusan,$angkatan) {
            $q->where("id_angkatan", $angkatan)->where("id_jurusan",$jurusan);
        })->with(["aspek_dpg"=>function($q){
            $q->where("nilai",1)->with("aspek4B");
        }])->whereHas("aspek_dpg", function($q){
            $q->where("nilai", 1);
        })->whereHas("parent", function($q) use($dari, $sampai){
            $q->whereBetween("tanggal_penilaian", [$dari, $sampai]);
        });

       
        $siswaN1a = CatatanSikap::with("kategori")->with("siswa")->whereHas('kategori', function($q) {
            $q->where("nilai", 1);
        })->whereBetween(DB::raw('DATE(tanggal)'), [$dari, $sampai])->whereHas("siswa", function($q) use($angkatan,$jurusan){
            $q->where("id_angkatan", $angkatan)->where("id_jurusan",$jurusan);
        })->get();

        if($params['semua'] == false){
            $siswaN1 = $siswaN1->whereHas("parent", function($q) use($params){
                $q->whereIn("id_penilai",$params["listguru"]);
            })->get();

            $siswaN1a = $siswaN1a->whereIn("id_penilai", $params['listguru'])->get();
        }else{
            $siswaN1 = $siswaN1->get();
            $siswaN1a = $siswaN1a->get();
        }

        $rincian = [];

        $rincian['sekolah'] = $siswaN1;
        $rincian['asrama'] = $siswaN1a;
