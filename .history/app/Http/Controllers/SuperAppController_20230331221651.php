...
class SuperAppController extends Controller
{
    ...
    public function profil(Request $req){
        $day = [
            ["label"=>"Senin", "lc"=>"senin"],
            ["label"=>"Selasa", "lc"=>"selasa"],
            ["label"=>"Rabu", "lc"=>"rabu"],
            ["label"=>"Kamis", "lc"=>"kamis"],
            ["label"=>"Jumat", "lc"=>"jumat"],
        ];

        return view("profil.index", ["hari"=>$day]);
    }
    ...
    public function storejadwal(Request $req){
       // dd($req);
        ...
        try {
            ...
            $jadwal->id_konselor=>$req->keterangan;
            $jadwal->id_konselor=>$req->minggu;
            $jadwal->id_konselors=>$req->bulan;
            $jadwal->id_konselor"=>date("Y");
            $jadwal->id_konselor = "Berjalan";
