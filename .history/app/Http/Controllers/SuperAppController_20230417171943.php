...
class SuperAppController extends Controller
{
    ...
    public function carisiswa(Request $req)
    {
        ...
        if(regularPermission(Auth::user()->getRoleNames()->toArray(),["Guru BK","Admin","Kepala Sekolah","Kesiswaan"])){
    ...
    public function profilsiswa($nis)
    {
        ...
        //validate
        $checker = TeacherHasTeaching::where("id_angkatan",$siswa->id_angkatan)->where("id_jurusan",$siswa->id_jurusan)->waliKelas(date("Y-m-d"));
        if(regularPermission(Auth::user()->getRoleNames()->toArray(),["Guru BK","Admin","Kesiswaan","Pamong Putra","Pamong Putri","Kepa"]))
