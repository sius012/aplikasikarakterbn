...
class SuperAppController extends Controller
{
    ...
    public function pengajuankonseling(Request $req){
        ...
        foreach($konselor as $ko){
            $idkonselor = $ko->id;
            $jadwalmingguini = DetailJK::whereHas("jadwal", function($q) use($idkonselor){
                $q->where("id_konselor",$ko->id)->where("tahun", Carbon::now()->year)->where("bulan",1)->where("minggu", Carbon::now()->weekOfMonth);
            })->where("hari", 3)->get();
