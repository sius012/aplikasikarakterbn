...
class SuperAppController extends Controller
{
    ...
    public function pengajuankonseling(Request $req){
        ...
        $finalreturn = ["konselor"=>$konselor];

        if($req->filled("id")){
            $validation = DetailJK::with("jadwal")->whereHas("jadwal", function($q) use($idkonselor){
                $q->where("id_jk",$idkonselor)->where("tahun", Carbon::now()->year)->where("bulan",1)->where("minggu", Carbon::now()->weekOfMonth);
            })->whereDoesntHave("bookedby")->where("hari", ">=",Carbon::now()->dayOfWeek)->get();
        }
