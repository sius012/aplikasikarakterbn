...
class SuperAppController extends Controller
{
    ...
    public function profilsiswa($nis)
    {
        ...
        $catatankonseling = PengajuanKonseling::where("nis_siswa", $nis)->where("status")->get();

        $siswa = Siswa::with(["detail","alamat"])->find($nis);

        return view("datasiswa.profilsiswa", ["siswa" => $siswa, "catataneraport"=>$catatanEraport, "catatankonseling"=>$catatankonseling]);
    }
    ...
    public function reservasikonseling(Request $req){
        $reservasikonseling = PengajuanKonseling::where("id_konselor",Auth::user()->id)->with("pengaju")->get();
        return view("bk.reservasikonseling", ["reservasikonseling"=>$reservasikonseling]);
    }
