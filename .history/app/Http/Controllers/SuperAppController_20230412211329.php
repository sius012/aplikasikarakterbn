...
class SuperAppController extends Controller
{
    ...
    public function profil(Request $req)
    {
        ...
        $jadwalkonseling = JadwalKonseling::withCount("detail_jk.bookedby")->where("id_konselor", Auth::user()->id)->orderBy("tahun", "desc")->orderBy("bulan", "desc")->orderBy("minggu", "desc");
        ...
        foreach($jadwalkonseling->get() as $jk){
            $hasBooked = DetailJK::where("id_jk",$jk->id_jk)->whereHave("bookedby")->get();
            dd($hasBooked);
        }

        return view("profil.index", ["hari" => $day, "jadwalkonseling" => $jadwalkonseling->get()]);
    }
