    ...
    public function tambahsiswa(Request $req)
    {
        ...
        try {
            ...
            if (!$find) {
                ...
                if($req->hasFile("foto")){
                    $datasiswa->foto_profil = $req->id_jurusan."_".$req->id_angkatan."_".$req->nis.".jpg";
                    $datasiswa->save();
                    
                    $file = Image::create($req->file('foto')->realPath());
                    $jurusan = Jurusan::find($req->id_jurusan);
                    $file->resize(function($f){
                        $f->fit(360,480);
                    })->save(public_path("siswa")."/".$req->id_angkatan."/".$jurusan->jurusan."/".$req->id_jurusan."_".$req->id_angkatan."_".$req->nis.".jpg");
                }
