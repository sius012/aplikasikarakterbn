...
class RKGController extends Controller
{
    ...
    public function hapusRKG($id){
        $pg = PenilaianGuru::with("detail_pg")->with("detail_pg.aspek_dpg")->find($id);
        $dpg = DetailPG::where("id_pg",$id)->first();
        $adpg = AspekDPG::where('id_dpg',$dpg->id_dpg);
       // dd($pg);
        if($pg->id_penilai == Auth::user()->id){
          //  dd($pg->detail_pg);
            $pg->detail_pg()->aspek_dpg()->delete();
            $pg->detail_pg->delete();
            $pg->delete();
        }
    }
