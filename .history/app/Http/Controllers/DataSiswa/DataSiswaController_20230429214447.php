...
class DataSiswaController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        foreach ($angkatan as $i => $akt) {
            $p = TeacherHasTeaching::where("id_guru", auth()->user()->id)->where("id_angkatan", $akt->id_angkatan)->count();
    ...
    public function datajurusan($angkatan)
    {
        ...
        $user = Auth::user();
        $jurusan = new Jurusan();
        $jurusan = $jurusan$q->where("id_angkatan", $angkatan);
        if (in_array($roles[0], ["Guru", "Wali Kelas", "K3"])) {
            $jurusan = $jurusan->whereHas("siswa", function($q) use($angkatan){
    ...
    //    / dd($jurusan);   
        $jurusan = $jurusan->map(function ($e, $i) use ($angkatan) {
            ...
            $visibility = checkClassPermission(["angkatan" => $angkatan, "jurusan" => $e->id_jurusan, "auth" => auth()]);
    ...
    public function updatesiswa(Request $req){
        $siswa  = Siswa::find($req->nis);
       // dd($req);
        $detail = DetailSiswa::where("nis", $req->nis)->first();
