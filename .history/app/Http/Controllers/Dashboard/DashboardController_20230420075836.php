    ...
    public function index(Request $req)
    {
        ...
        foreach($kategori as $k =>$ktg){
            $jurusan = Jurusan::whereHas("siswa.angkatan", function ($q) {
                ...
                $q->whereHas('catatan_sikap', function ($j) {
                    $j->where("id_kategori", $ktg->id_kategori)$j->whereHas("kategori", function($k){
                        $k->
                    })
           ...
           "), ">", date("Y"))->orWhere(
            ...
            array_push($case, ["jeniskasus"=>$ktg->kategori,"data"=>$angkatan->toArray()]);
