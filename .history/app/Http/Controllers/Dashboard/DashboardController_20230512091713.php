...
class DashboardController extends Controller
{
    public function index(Request $req)
    {
        ...
        foreach($kategori as $k =>$ktg){
            ...
            $angkatan = Angkatan::aktif(->with(["siswa.catatan_sikap" => function ($j) use ($req,$ktg){
                $j->where("id_kategori", $ktg->id_kategori);
                if($req->has('bulan')){
                    $j->whereMonth("tanggal", Carbon::parse($req->bulan)->month)->whereYear("tanggal", Carbon::parse($req->bulan)->year);
                 }else{
       ...
       $jumlahsiswa = $this->jumlahSiswaKelas();
        //dd($jumlahsiswa);

        if ($req->has("requestJson")) {
            return json_encode(['statistika'=>$case,'jumlahsiswa'=>$jumlahsiswa]);
        } else {
    ...
    public function jumlahSiswaKelas($kelas=null){
        ...
        $kelas = $kelas->map(function($e){
            ...
            $count = Jurusan::withCount(["siswa"=>function($q) use($e){
                $q->where("id_angkatan",$e->id_angkatan);
            }])->whereHas("siswa", function($q)use($e){
                $q->where("id_angkatan",$e->id_angkatan);
            })->get();

            $count = $count->map(function($e){
                return $e->siswa_count;
            });
           
            return [
                'kelas'=> $e->kelas(),
                'label'=>$jurusan,
                'jumlah'=>$count->toArray()
            ];
        });
