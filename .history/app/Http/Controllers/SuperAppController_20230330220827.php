...
class SuperAppController extends Controller
{
    ...
    public function profilsiswa($nis)
    {
        ...
        return view("datasiswa.profilsiswa", ["siswa" => $siswa, "catataneraport"=>$catatanEraport]);
    }
    ...
    public function loginsiswaattempt(Request $req){
        $siswacount = Siswa::where("nis",$req->nis)->where("password",Hash::make($req->password))->count();
        return redirect()->back()->with("error", "Kredensial tidak cocok,pastikan nis dan kata sandi benar ya");
        $siswa =$siswacount = Siswa::where("nis",$req->nis)->where("password",Hash::make($req->password))->count();
        Auth::guard("siswa")->login($siswa);
    }

    public function pengajuankonseling(Request $req){
        $konselor = User::role("Guru BK")->get();
        return view("siswa.pengajuankonseling", ["konselor"=>$konselor]);
    }
